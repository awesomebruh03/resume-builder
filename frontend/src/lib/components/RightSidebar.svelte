...
<Sidebar side="right">
  ...
  >
    <section>
      <div class="flex items-center gap-2 mb-3">
        <iconify-icon icon="mdi:view-grid" class="text-amber-500"
        ></iconify-icon>
      ...
      <div class="grid grid-cols-2 gap-3">
        {#each templates as t}
          ...
          >
            ...
            <div
              class="absolute bottom-0 w-full bg-black/80 backdrop-blur-sm py-1 px-2 border-t border-white/10"
            >
              <span
            ...
            {#if $design.template === t.id}
              <div
                class="absolute top-2 right-2 w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,1)]"
              ></div>
            {/if}
          </button>
        {/each}
    ...
    <section>
      ...
      <div class="flex flex-col gap-4">
        <div class="flex flex-wrap gap-3 items-center">
          {#each colors as color}
            <button
              ...
              style="background-color: {color}; border-color: {$design.themeColor ===
              color
                ? 'white'
                : 'transparent'};"
            ...
            >
              {#if $design.themeColor === color}
                <iconify-icon
                  icon="mdi:check"
                  class="text-white text-xs drop-shadow-md"
                ></iconify-icon>
              {/if}
            </button>
          {/each}
        ...
        <div
          class="flex items-center justify-between bg-[#18181b] p-2 rounded border border-[#27272a]"
        >
          <span class="text-xs text-gray-500">Active Color:</span>
          <div
            class="w-16 h-5 rounded border border-gray-600 shadow-inner transition-colors duration-300"
            style="background-color: {$design.themeColor}"
          ></div>
        </div>
      </div>
    </section>
    ...
    <section>
      <div class="flex items-center gap-2 mb-3">
        <iconify-icon icon="mdi:file-document-outline" class="text-amber-500"
        ></iconify-icon>
        <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest">
          Page Settings
        </h3>
      ...
      <div class="grid grid-cols-2 gap-3">
        <button
          class="btn btn-sm {$design.pageSize === 'A4'
            ? 'btn-primary text-white'
            : 'btn-outline border-gray-700 text-gray-400'}"
          on:click={() => ($design.pageSize = 'A4')}>A4 (ISO)</button
        >
        <button
          class="btn btn-sm {$design.pageSize === 'Letter'
            ? 'btn-primary text-white'
            : 'btn-outline border-gray-700 text-gray-400'}"
    ...
    <section>
      <div class="flex items-center gap-2 mb-3">
        <iconify-icon icon="mdi:format-font" class="text-amber-500"
        ></iconify-icon>
        <h3 class="text-xs font-bold text-gray-400 uppercase tracking-widest">
          Typography
        </h3>
      ...
      <div class="flex flex-col gap-5">
        <div class="form-control w-full">
          <label class="label py-1"
            ><span class="label-text text-xs text-gray-400">Font Family</span
            ></label
          >
          <select
          ...
          >
            {#each fontGroups as group}
              <optgroup
                label={group.label}
                class="bg-[#18181b] text-gray-500 font-bold uppercase tracking-wider text-[10px]"
              >
                {#each group.options as font}
                  <option
                    value={font.value}
                    class="text-gray-200 font-normal text-sm normal-case"
                    >{font.name}</option
                  >
                {/each}
              </optgroup>
        ...
        <div class="form-control w-full mt-2">
          ...
          >
            <button
              class="w-full flex items-center justify-between bg-[#18181b] border border-[#27272a] p-2 rounded hover:border-gray-500 transition-colors"
              title="Change Body Text Color"
            >
              <span class="text-xs text-gray-500">Select Color</span>
              <div
                class="w-12 h-5 rounded border border-gray-600 shadow-sm"
                style="background-color: {$design.textColor}"
              ></div>
            </button>
          </ColorPicker>
        </div>
        ...
        {#each $design.sectionOrder as section (section.id)}
          ...
          >
            <div class="flex items-center gap-3">
              <iconify-icon
                icon="mdi:drag"
                class="text-gray-600 group-hover:text-amber-500 transition-colors text-lg"
              ></iconify-icon>
              <span
                class="text-xs font-bold text-gray-300 group-hover:text-white"
                >{section.name}</span
              >
            </div>
            <button
              class="btn btn-xs btn-circle btn-ghost text-gray-600 hover:text-white"
              ><iconify-icon icon="mdi:eye"></iconify-icon></button
            >
          </div>
        {/each}
      </div>
  ...
  <div
    slot="footer"
    class="p-4 border-t border-[#27272a] bg-[#0e0e0e]/95 backdrop-blur absolute bottom-0 w-full z-10 flex flex-col gap-2"
  >
    <button
    ...
    <div class="grid grid-cols-2 gap-2">
      <button
        class="btn btn-xs btn-outline border-gray-700 text-gray-400 hover:text-white"
        >Save JSON</button
      >
      <button
        class="btn btn-xs btn-outline border-gray-700 text-gray-400 hover:text-white"
